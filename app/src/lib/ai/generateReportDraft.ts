...
export async function translateReportComment(opts: {
  ...
  if (!t) return "";
  if (opts.fromLanguage === opts.toLanguage) return t;
...
export async function generateSchoolReportDraftPair(opts: {
  ...
  inputs: ReportInputs;
  extraNotes?: string;
}): Promise<{ pdfBody: string; teacherPreview: string }> {
  const common = {
  ...
  const pdfBody = await generateSchoolReportDraft({
    ...common,
  ...
  if (opts.pdfLanguage === opts.teacherLanguage) {
    ...
    teacherPreview = await translateReportComment({
      text: pdfBody,
      fromLanguage: opts.pdfLanguage,
      toLanguage: opts.teacherLanguage,
    });
  }
  return { pdfBody, teacherPreview };
}
